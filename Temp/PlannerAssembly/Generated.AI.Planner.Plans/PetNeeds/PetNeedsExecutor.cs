        ...
        protected override void Act(ActionKey actionKey)
        {
            ...
            foreach (var argument in argumentMapping)
            {
                ...
                if (split.Length > 1) // argument is a trait
                {
                    switch (split[1])
                    {
                        case nameof(PetAgent):
                            var traitPetAgent = stateData.GetTraitOnObjectAtIndex<PetAgent>(traitBasedObjectIndex);
                            arguments[i] = split.Length == 3 ? traitPetAgent.GetField(split[2]) : traitPetAgent;
                            break;
